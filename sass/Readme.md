...
folder are mapped to the application's root namespace, 'AttributeUI'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "AttributeUI"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'AttributeUI' in the same way
as `"AttributeUI/sass/src"`.
